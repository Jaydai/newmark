"""CU1 — Lecture Presse → Excel (Transaction extraction)
...
from transaction_analysis import classify_and_extract_transaction
from excel_builder import build_transaction_excel

app = FastAPI(title="CU1 — Lecture Presse Excel")
...
# ============================================================
def run_pipeline():
    """Scrape Business Immo → classify with AI → build Excel."""
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                for i, article in enumerate(all_articles):
                    ...
                    log("Article %d/%d: got %d chars, classifying...", i+1, len(all_articles), len(content))

                    result = classify_and_extract_transaction(
                        title=article["title"],
                        content=content,
                    ...
                    result["contenu_article"] = content

                    if result.get("est_transaction"):
                        result.setdefault("source", article["source"])
                        result.setdefault("url", article["url"])
                        result.setdefault("titre_article", article["title"])
                        transactions.append(result)
                        log("Article %d/%d: TRANSACTION — %s", i+1, len(all_articles), article["title"][:50])
                    else:
                        result.setdefault("source", article["source"])
                        result.setdefault("url", article["url"])
                        result.setdefault("titre", article["title"])
                        other_articles.append(result)
                        log("Article %d/%d: not a transaction", i+1, len(all_articles))

                    time.sleep(0.5)
            finally:
                browser.close()

        log("Done: %d transactions, %d other from %d articles", len(transactions), len(other_articles), len(all_articles))

        xlsx_base64 = build_transaction_excel(transactions, other_articles)

        return {
            "date": datetime.now().strftime("%d/%m/%Y %H:%M"),
            "nb_articles": len(all_articles),
            "nb_transactions": len(transactions),
            "nb_other": len(other_articles),
...
async def start_scrape():
    ...
    job_id = start_job(run_pipeline)
    logging.warning("D07-CU1 pipeline started — job %s", job_id)
...
@app.get("/version")
async def version():
    return {"agent": "cu1-lecture-presse", "build": BUILD_VERSION}
