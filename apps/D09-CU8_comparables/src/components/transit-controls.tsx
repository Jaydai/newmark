...
export default function TransitControls({ mapRef, view }: TransitControlsProps) {
  ...
  const layersRef = useRef<Record<LayerType, L.LayerGroup | null>>({
    lines: null,
    stations: null,
    labels: null,
    numbers: null,
  ...
  return (
    <div className="fixed bottom-[100px] left-6 z-[900] flex rounded-[var(--radius)] overflow-hidden shadow-sm">
      {btns.map((b) => (
        <button
          key={b.type}
