...
const HASH_PATH = path.join(PUBLIC_DATA, "transactions-hash.json");

// Import the existing parse-xlsx from lib/
...
function writePayload(payload) {
  const jsonPath = path.join(PUBLIC_DATA, "transactions.json");
  fs.writeFileSync(jsonPath, JSON.stringify(payload));
  fs.writeFileSync(HASH_PATH, JSON.stringify({ hash: payload.hash || "" }));
}
...
function buildTransactions() {
  ...
  if (fs.existsSync(XLSX_PATH)) {
  ...
  } else if (fs.existsSync(path.join(PUBLIC_DATA, "transactions.json"))) {
    const existing = JSON.parse(fs.readFileSync(path.join(PUBLIC_DATA, "transactions.json"), "utf8"));
    ...
    console.log("No infos.xlsx — using existing transactions.json");
