...
function bubbleScore(tx: Transaction): number {
  return (tx.annualHeadlineRent ?? 0) + (tx.totalSurfaceSqm ?? 0) * 10;
}

interface TxBubbleSceneProps {
  active: boolean;
  visible: boolean;
  data: Transaction[];
...
export default function TxBubbleScene({
  active,
  visible,
  data,
  mapRef,
  ...
  const getBubbleTransactions = useCallback((padRatio = 0) => {
    const map = mapRef.current;
    const geocoded = data.filter((tx) => hasCoords(tx.lat, tx.lng));
    if (!map || !geocoded.length) return { shown: [] as Transaction[], total: 0 };

    const bounds = map.getBounds().pad(padRatio);
    const inView = geocoded.filter((tx) => bounds.contains([tx.lat!, tx.lng!]));
    if (!inView.length) return { shown: [] as Transaction[], total: 0 };
  ...
  useEffect(() => {
    ...
    const renderBubbles = () => {
      ...
      const topOffset = 0;
      const geocoded = data.filter((tx) => hasCoords(tx.lat, tx.lng));
      const { shown: visible, total: totalInView } = getBubbleTransactions();

      if (!data.length) {
        scene.innerHTML = '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:14px;color:var(--text-secondary);pointer-events:auto">Aucune transaction à afficher.</div>';
        return;
      }

      if (!geocoded.length) {
        scene.innerHTML = '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:14px;color:var(--text-secondary);pointer-events:auto">Aucune transaction géolocalisée.</div>';
        return;
      }

      if (!visible.length) {
        scene.innerHTML = '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:14px;color:var(--text-secondary);pointer-events:auto">Aucune transaction visible dans cette zone.</div>';
        return;
      }

      currentBubbleIdsRef.current = visible.map((tx) => tx.id).join(",");

      const markerToScene = (tx: Transaction) => {
        const mapEl = map.getContainer();
        const mapRect = mapEl.getBoundingClientRect();
        const point = map.latLngToContainerPoint([tx.lat!, tx.lng!]);
        return {
      ...
      const radii = visible.map(() => Math.max(cardWidth, cardHeight) / 2);
      const padding = 20;
      const left = padding;
      ...
      const markerScreens = visible.map((tx) => markerToScene(tx));
      ...
      const positions = visible.map((_, idx) => perimeterToXY(slotDistances[idx]));
      ...
      visible.forEach((tx, idx) => {
        const saved = savedPositionsRef.current[tx.id];
        if (saved) positions[idx] = { ...saved };
      });

      const svgElements = visible.map(() => {
      ...
      const onMapShift = () => {
        ...
        moveDebounceRef.current = window.setTimeout(() => {
          const nextIds = getBubbleTransactions(0.5).shown.map((tx) => tx.id).join(",");
      ...
      visible.forEach((tx, idx) => {
        ...
        bubble.dataset.txBubbleCard = "true";
        bubble.tabIndex = 0;
        bubble.setAttribute("role", "button");
        bubble.setAttribute("aria-label", `Ouvrir le détail de ${tx.newTenant || tx.enseigne || "cette transaction"}`);
        ...
        bubble.innerHTML = [
          `<div class="tx-bub-badge" style="background:var(--map-blue);color:#fff;font-size:6px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;padding:1px 6px;border-radius:20px;margin-bottom:2px;white-space:nowrap">TRANSACTION${tx.year ? ` · ${esc(tx.year)}` : ""}</div>`,
          `<div class="tx-bub-name" style="font-size:${nameSize};font-weight:700;color:var(--primary);line-height:1.15;margin-bottom:1px;max-width:90%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(tx.newTenant || tx.enseigne || "-")}</div>`,
          `<div class="tx-bub-area" style="font-size:7px;color:var(--text-secondary);margin-bottom:2px;max-width:90%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(tx.area || "")}</div>`,
          `<div class="tx-bub-row" style="display:flex;justify-content:space-between;width:85%;font-size:${rowSize};padding:0"><span class="tx-bub-label" style="color:var(--text-secondary)">Surface</span><span class="tx-bub-value" style="font-weight:700;color:var(--primary)">${esc(fmtSurface(tx.totalSurfaceSqm))}</span></div>`,
          `<div class="tx-bub-row" style="display:flex;justify-content:space-between;width:85%;font-size:${rowSize};padding:0"><span class="tx-bub-label" style="color:var(--text-secondary)">Loyer</span><span class="tx-bub-value" style="font-weight:700;color:var(--primary)">${esc(fmtEuro(tx.annualHeadlineRent))}</span></div>`,
          `<div class="tx-bub-row" style="display:flex;justify-content:space-between;width:85%;font-size:${rowSize};padding:0"><span class="tx-bub-label" style="color:var(--text-secondary)">€/m² pond.</span><span class="tx-bub-value" style="font-weight:700;color:var(--primary)">${esc(fmtLoyer(tx.rentSqmWeighted))}</span></div>`,
        ].join("");
        ...
        bubble.addEventListener("pointerdown", (event) => {
          ...
          const onUp = () => {
            ...
            if (dragged) {
              savedPositionsRef.current[tx.id] = { ...positions[idx] };
            } else {
              onSelect(tx);
            }
          };
      ...
      watermark.innerHTML = '<div style="font-size:14px;font-weight:600;letter-spacing:3px;color:rgba(0,0,0,.06)">NEWMARK</div><div style="font-size:8px;letter-spacing:2px;text-transform:uppercase;color:rgba(0,0,0,.04);margin-top:2px">Carte des Transactions</div>';
  ...
  }, [active, analyticsWidth, clearBubbles, data, getBubbleTransactions, mapRef, onSelect]);

  return (
    <div
      ref={sceneRef}
      data-tx-bubble-scene
