...
import { parseExcelBuffer } from "@/lib/parse-xlsx-client";
...
interface ImportDialogProps {
  open: boolean;
  onClose: () => void;
  onImport: (transactions: Transaction[]) => { added: number; ignored: number };
}
...
export default function ImportDialog({ open, onClose, onImport }: ImportDialogProps) {
  ...
  const handleBuffer = useCallback(async (buffer: ArrayBuffer, fileName: string) => {
    ...
    setError(null);
    try {
      const txs = await parseExcelBuffer(buffer);
      if (txs.length === 0) {
        setError("Aucune transaction trouvée dans ce fichier. Vérifiez les en-têtes de colonnes.");
        return;
      }
      const importResult = onImport(txs);
      if (importResult.added === 0) {
        setError("Toutes les transactions de ce fichier sont déjà présentes.");
        return;
      }
    ...
    <div className="fixed inset-0 z-[3000] bg-black/60 backdrop-blur-[4px] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <h3 className="text-sm font-bold">Importer des transactions</h3>
        ...
        <div className="px-5 pt-4 shrink-0">
          <div className="flex items-center gap-1 p-1 rounded-xl bg-surface-alt w-fit">
            <button
              onClick={() => { setSourceMode("local"); setError(null); setResult(null); }}
              className={clsx(
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {result && (
            <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 text-green-700 text-sm mb-4">
              ...
              <div>
                <p className="font-semibold">{result.added} transactions importées</p>
