...
export async function renderLeafletTilesToCanvas(
  mapEl: HTMLElement,
  scale = 2
    ...
    tiles.map(async (tile) => {
      ...
      try {
        ...
        try {
          ...
          console.warn(`[leaflet-export] Skipping tile (CORS/tainted): ${tile.src}`);
        }
      }
    })
  );
...
export function drawLeafletMarkersToCanvas(ctx: CanvasRenderingContext2D, mapEl: HTMLElement): void {
  ...
  markers.forEach((marker) => {
    ...
    const root = marker.firstElementChild instanceof HTMLElement ? marker.firstElementChild : marker;
    const styles = getComputedStyle(root);
    const number = root.textContent?.trim() ?? "";

    ctx.save();
    ctx.beginPath();
    ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
    ctx.fillStyle = styles.backgroundColor || "#2d8c5a";
    ctx.fill();

    ctx.strokeStyle = styles.borderColor || "rgba(255,255,255,0.85)";
    ctx.lineWidth = Number.parseFloat(styles.borderWidth || "0") || 2;
    ctx.stroke();

    if (number) {
      ctx.fillStyle = "#ffffff";
      ctx.font = `700 ${Math.round(radius)}px "DM Sans", sans-serif`;
...
export function drawLeafletCanvasOverlayToCanvas(ctx: CanvasRenderingContext2D, mapEl: HTMLElement): void {
  const overlayCanvas = mapEl.querySelector<HTMLCanvasElement>(".leaflet-overlay-pane canvas");
...
export async function rasterizeLeafletSvgOverlay(
  mapEl: HTMLElement
): Promise<{ img: HTMLImageElement; x: number; y: number; w: number; h: number } | null> {
  const overlaySvg = mapEl.querySelector<SVGSVGElement>(".leaflet-overlay-pane svg");
  if (!overlaySvg || !overlaySvg.childElementCount) return null;
