...
function FitBounds({ data, trigger }: { data: OffreRetail[]; trigger: number }) {
  const map = useMap();
...
export default function OrMapInner({ data, fitTrigger, onSelect, mapRef, sidebarWidth, analyticsWidth }: OrMapInnerProps) {
  const markers = useMemo(() => {
    return data.filter((item) => hasCoords(item.lat, item.lng));
  }, [data]);
  ...
  useEffect(() => {
    ...
    markers.forEach((item) => {
      const m = L.marker([item.lat!, item.lng!], { icon: orIcon }).addTo(map);
