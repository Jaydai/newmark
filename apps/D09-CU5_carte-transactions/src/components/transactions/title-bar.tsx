...
export default function TxTitleBar({
  view, onViewChange, yearFrom, yearTo, onYearFromChange, onYearToChange,
  allYears, analyticsOpen, onToggleAnalytics, themePickerSlot,
}: TitleBarProps) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-[var(--bar-h)] bg-white border-b border-border text-primary px-4 z-[1000] flex items-center gap-4">
      ...
      <div className="flex items-center gap-2 text-[11px]">
        <span className="text-text-secondary font-medium">Années</span>
        <select value={yearFrom} onChange={(e) => onYearFromChange(e.target.value)}
          className="border border-border-input rounded-md px-2 py-1 text-[11px] bg-white cursor-pointer focus:outline-none focus:border-accent">
          <option value="">Min</option>
          {allYears.map((y) => <option key={y} value={y}>{y}</option>)}
        </select>
        <span className="text-text-tertiary">–</span>
        <select value={yearTo} onChange={(e) => onYearToChange(e.target.value)}
          className="border border-border-input rounded-md px-2 py-1 text-[11px] bg-white cursor-pointer focus:outline-none focus:border-accent">
