...
export default function AnalyticsPanel({ open, onClose, data }: AnalyticsPanelProps) {
  ...
  const droitEntreeRef = useRef<SVGSVGElement>(null);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      renderDroitEntreeChart(droitEntreeRef.current, data);
      renderSurfaceChart(surfaceRef.current, data);
      renderTypeChart(typeRef.current, data);
    }, 100);
  ...
  return (
    <div className={`fixed top-[var(--bar-h)] right-0 bottom-0 w-[620px] max-w-[90vw] bg-white border-l border-border shadow-[-8px_0_40px_rgba(0,0,0,.1)] z-[950] flex flex-col transition-transform duration-350 ease-[cubic-bezier(.4,0,.2,1)] ${open ? "translate-x-0" : "translate-x-full"}`}>
      <div className="px-5 py-4 border-b border-border flex items-center justify-between shrink-0">
        <h2 className="text-sm font-bold">Analyse des Offres Retail</h2>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-8">
        <ChartSection title="Loyer/m² par Ville">
        ...
        <ChartSection title="Droit d'Entrée">
          <svg ref={droitEntreeRef} className="w-full" style={{ height: 220 }} />
        </ChartSection>
        ...
        <ChartSection title="Répartition par Type">
          <svg ref={typeRef} className="w-full" style={{ height: 220 }} />
        </ChartSection>
      </div>
    </div>
  );
...
function renderRentChart(el: SVGSVGElement | null, data: OffreRetail[]) {
  if (!el) return;
  ...
  const withRent = data.filter((t) => t.loyerM2 != null);
  if (!withRent.length) { svg.append("text").attr("x", w / 2).attr("y", h / 2).attr("text-anchor", "middle").attr("fill", "#999").attr("font-size", 11).text("Aucune donnée"); return; }

  const byCity = d3.rollup(withRent, (v) => d3.mean(v, (t) => t.loyerM2 ?? 0)!, (t) => t.city || "?");
  const entries = [...byCity.entries()].sort((a, b) => a[0].localeCompare(b[0]));
  ...
  const allVals = withRent.map((t) => t.loyerM2 ?? 0).sort(d3.ascending);
...
function renderEnseigneChart(el: SVGSVGElement | null, data: OffreRetail[]) {
  if (!el) return;
  const svg = d3.select(el);
  svg.selectAll("*").remove();
  const w = el.clientWidth, h = 280;

  const counts = d3.rollup(data, (v) => v.length, (t) => t.enseigne || "?");
  ...
  g.append("text").attr("text-anchor", "middle").attr("dy", "1.2em").attr("font-size", 9).attr("fill", "#888").text("offres");
...
function renderDroitEntreeChart(el: SVGSVGElement | null, data: OffreRetail[]) {
  if (!el) return;
  const svg = d3.select(el);
  svg.selectAll("*").remove();
  const w = el.clientWidth, h = 220;
  ...
  const withDE = data.filter((t) => t.droitEntree != null && t.droitEntree > 0);
  if (!withDE.length) { svg.append("text").attr("x", w / 2).attr("y", h / 2).attr("text-anchor", "middle").attr("fill", "#999").attr("font-size", 11).text("Aucune donnée"); return; }

  const byAddr = d3.rollup(withDE, (v) => d3.mean(v, (t) => t.droitEntree!)!, (t) => t.adresse || "?");
  const entries = [...byAddr.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
...
function renderSurfaceChart(el: SVGSVGElement | null, data: OffreRetail[]) {
  if (!el) return;
  const svg = d3.select(el);
  svg.selectAll("*").remove();
  const w = el.clientWidth, h = 220;
  ...
  const vals = data.map((t) => t.surface).filter((v): v is number => v != null && v > 0).sort(d3.ascending);
...
function renderTypeChart(el: SVGSVGElement | null, data: OffreRetail[]) {
  if (!el) return;
  const svg = d3.select(el);
  svg.selectAll("*").remove();
  const w = el.clientWidth, h = 220;
  ...
  const counts = d3.rollup(data, (v) => v.length, (t) => t.transactionType || "?");
